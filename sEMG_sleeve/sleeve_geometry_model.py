...
class ChannelToCylinder(nn.Module):
    ...
    @property
    def n_valid(self):
        return int(self.valid_channels.numel())
...
class SleeveGeometryAttentionModel(nn.Module):
    def __init__(
        ...
        valid_linear_idx = (
            self.mapper.valid_rings * self.mapper.n_slots + self.mapper.valid_slots
        )
        self.register_buffer("valid_linear_idx", valid_linear_idx)
    ...
    def _geometry_frontend(self, x):
        ...
        x = x[:, :, self.valid_linear_idx, :]
        x = x.reshape(bsz, self.geom_ch * self.mapper.n_valid, timesteps)
        return self.pre(x)
